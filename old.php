...
<title>Phố đồ cũ - <?php require_once('z-include/title_p2.php'); ?>
</head>
    	...
    	<div id="primary">
            ...
            <div class="div_frame_ctrl">
                <ul class="ul_frame_ctrl">
					...
					?>
                    >
                    	<a href="old.php?type=<?php echo $row->NameNone; ?>"><?php echo $row->Name; ?></a>
                    </li>
					<?php
					}
                    ?>
                ...
                <div class="div_frame_ctrl_content">
					...
					while ($row3 = mysql_fetch_object($que3))
					{
						...
						if ($gtype2 == $NameNone)
							echo '<a id="current" href="old.php?type='.$gtype.'&type2='.$NameNone.'">'.$Name2.'</a>';
						else
							echo '<a href="old.php?type='.$gtype.'&type2='.$NameNone.'">'.$Name2.'</a>';
					}
					?>
                    <div class="clear"></div>
                </div>
			...
			<div class="div_products">
				...
				if ($_SESSION['lo'] == 'Tất cả' || $_SESSION['lo'] == '')
				{
					...
					{
						$que_5 = que("SELECT * FROM products WHERE idOldNew = 2 AND PriProduct = 1 AND Active = 1");
						$que5 = que("SELECT * FROM products WHERE idOldNew = 2 AND PriProduct = 1 AND Active = 1 ORDER BY ".$gsort." ".$tsort." LIMIT ".$start.",".$perPage);
					}
					elseif ($row4->idTypeProduct2 == '')
					{
						$que_5 = que("SELECT * FROM products WHERE idTypeProduct ='".$row2->idTypeProduct."' AND Active = 1 AND idOldNew = 2 AND PriProduct = 1");
						$que5 = que("SELECT * FROM products WHERE idTypeProduct ='".$row2->idTypeProduct."' AND Active = 1 AND idOldNew = 2 AND PriProduct = 1 ORDER BY ".$gsort." ".$tsort." LIMIT ".$start.",".$perPage);
					}
					else
					{
						$que_5 = que("SELECT * FROM products WHERE idTypeProduct ='".$row2->idTypeProduct."' AND Active = 1 AND idTypeProduct2 = '".$row4->idTypeProduct2."' AND idOldNew = 2 AND PriProduct = 1");
						$que5 = que("SELECT * FROM products WHERE idTypeProduct ='".$row2->idTypeProduct."' AND Active = 1 AND idTypeProduct2 = '".$row4->idTypeProduct2."' AND idOldNew = 2 AND PriProduct = 1 ORDER BY ".$gsort." ".$tsort." LIMIT ".$start.",".$perPage);
					}
				}
				else
				{
					if ($row2->idTypeProduct == '')
					{
						$que_5 = que("SELECT * FROM products WHERE idOldNew = 2 AND PriProduct = 1 AND Active = 1 AND idLocation = '".$idLocation."'");
						$que5 = que("SELECT * FROM products WHERE idOldNew = 2 AND PriProduct = 1 AND Active = 1 AND idLocation = '".$idLocation."' ORDER BY ".$gsort." ".$tsort." LIMIT ".$start.",".$perPage);
					}
					elseif ($row4->idTypeProduct2 == '')
					{
						$que_5 = que("SELECT * FROM products WHERE idTypeProduct ='".$row2->idTypeProduct."' AND Active = 1 AND idLocation = '".$idLocation."' AND idOldNew = 2 AND PriProduct = 1");
						$que5 = que("SELECT * FROM products WHERE idTypeProduct ='".$row2->idTypeProduct."' AND Active = 1 AND idLocation = '".$idLocation."' AND idOldNew = 2 AND PriProduct = 1 ORDER BY ".$gsort." ".$tsort." LIMIT ".$start.",".$perPage);
					}
					else
					{
						$que_5 = que("SELECT * FROM products WHERE idTypeProduct ='".$row2->idTypeProduct."' AND Active = 1 AND idLocation = '".$idLocation."' AND idTypeProduct2 = '".$row4->idTypeProduct2."' AND idOldNew = 2 AND PriProduct = 1");
						$que5 = que("SELECT * FROM products WHERE idTypeProduct ='".$row2->idTypeProduct."' AND Active = 1 AND idLocation = '".$idLocation."' AND idTypeProduct2 = '".$row4->idTypeProduct2."' AND idOldNew = 2 AND PriProduct = 1 ORDER BY ".$gsort." ".$tsort." LIMIT ".$start.",".$perPage);
					}
				}
