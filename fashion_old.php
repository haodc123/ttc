    ...
    <div id="eprimary">
    	<div id="primary">
			...
			<div class="div_products">
				...
				if ($gsort == 'Date')
				{
				$expDate = date("Y-n-d H:i:s", strtotime("-3 hours"));
				$que_pr = que("SELECT * FROM products WHERE idOldNew = 1 AND PriProduct = 1");
					while ($row_pr = mysql_fetch_object($que_pr))
					{
						if (strtotime($row_pr->Up) < strtotime($expDate))
						{
							$queu = que("UPDATE products SET Up = 0 WHERE idProduct = '".$row_pr->idProduct."'");
						}
					}
				}
				...
				{
					$que_5 = que("SELECT * FROM products WHERE idOldNew = 1 AND PriProduct = 1");
					$que5 = que("SELECT * FROM products WHERE idOldNew = 1 AND PriProduct = 1 ORDER BY ".$gsort." ".$tsort." LIMIT ".$start.",".$perPage);
					if ($gsort == 'Date')
					{
					$que_5 = que("SELECT * FROM products WHERE idOldNew = 1 AND PriProduct = 1");
					$que5 = que("SELECT * FROM products WHERE idOldNew = 1 AND PriProduct = 1 ORDER BY Up DESC, Date DESC LIMIT ".$start.",".$perPage);
					}
				}
				elseif ($row4->idTypeProduct2 == '')
				{
					$que_5 = que("SELECT * FROM products WHERE idTypeProduct ='".$row2->idTypeProduct."' AND idOldNew = 1 AND PriProduct = 1");
					$que5 = que("SELECT * FROM products WHERE idTypeProduct ='".$row2->idTypeProduct."' AND idOldNew = 1 AND PriProduct = 1 ORDER BY ".$gsort." ".$tsort." LIMIT ".$start.",".$perPage);
					if ($gsort == 'Date')
					{
					$que_5 = que("SELECT * FROM products WHERE idTypeProduct ='".$row2->idTypeProduct."' AND idOldNew = 1 AND PriProduct = 1");
					$que5 = que("SELECT * FROM products WHERE idTypeProduct ='".$row2->idTypeProduct."' AND idOldNew = 1 AND PriProduct = 1 ORDER BY Up DESC, Date DESC LIMIT ".$start.",".$perPage);
					}
				}
				else
				{
					$que_5 = que("SELECT * FROM products WHERE idTypeProduct ='".$row2->idTypeProduct."' AND idTypeProduct2 = '".$row4->idTypeProduct2."' AND idOldNew = 1 AND PriProduct = 1");
					$que5 = que("SELECT * FROM products WHERE idTypeProduct ='".$row2->idTypeProduct."' AND idTypeProduct2 = '".$row4->idTypeProduct2."' AND idOldNew = 1 AND PriProduct = 1 ORDER BY ".$gsort." ".$tsort." LIMIT ".$start.",".$perPage);
					if ($gsort == 'Date')
					{
					$que_5 = que("SELECT * FROM products WHERE idTypeProduct ='".$row2->idTypeProduct."' AND idTypeProduct2 = '".$row4->idTypeProduct2."' AND idOldNew = 1 AND PriProduct = 1");
					$que5 = que("SELECT * FROM products WHERE idTypeProduct ='".$row2->idTypeProduct."' AND idTypeProduct2 = '".$row4->idTypeProduct2."' AND idOldNew = 1 AND PriProduct = 1 ORDER BY Up DESC, Date DESC LIMIT ".$start.",".$perPage);
					}
				}
