...
if ($row_uc->idUser == $row_up->idUser)
{
	...
	{
?>
        	<h1 class="h1_up_title">Sửa thông tin Shop hàng :</h1>
        	<?php
			$que_uc = que("SELECT * FROM users WHERE NameNone = '".$_SESSION['NameNone']."'");
			$row_uc = mysql_fetch_object($que_uc);
			if ($row_uc->nEdit > 0)
    ...
    ***************************************************/
    ...
    oEdit1.REPLACE("fupcontent");
        ...
        <a class="add_img" href="edit_post_addimg.php?step=1&name=<?php echo $gname; ?>">Bạn muốn up thêm hình ảnh ? Bấm vào đây</a>

<?php
			}
			else
			{
				echo '<p>Xin lỗi bạn, bạn đã hết lượt Sửa bài cho phép !</p>';
			}
	}
	elseif ($gdo == 'finish')
	{
...
elseif ($gdo == 'delfinish')
{
	echo '<p>Bạn đã xóa thành công</p><br />';
}		
else
