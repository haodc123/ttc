...
<title>Khuyến mãi hàng thời trang - <?php require_once('z-include/title_p2.php'); ?>
</head>
    	...
    	<div id="primary">
        	<div class="div_content">
            		...
            		<div class="promote_title">
                        ...
                        <h1><a href="#">Các sản phẩm khuyến mãi</a></h1>
                    </div>
            ...
            <div class="div_frame_ctrl">
                <ul class="ul_frame_ctrl">
					...
					?>
                    >
                    	<a href="promote.php?type=<?php echo $row->NameNone; ?>"><?php echo $row->Name; ?></a>
                    </li>
					<?php
					}
                    ?>
                ...
                <div class="div_frame_ctrl_content">
					...
					while ($row3 = mysql_fetch_object($que3))
					{
						...
						if ($gtype2 == $NameNone)
							echo '<a id="current" href="promote.php?type='.$gtype.'&type2='.$NameNone.'">'.$Name2.'</a>';
						else
							echo '<a href="promote.php?type='.$gtype.'&type2='.$NameNone.'">'.$Name2.'</a>';
					}
					?>
                    <div class="clear"></div>
                </div>
			...
			<div class="div_products">
				...
				{
					$que_5 = que("SELECT * FROM products WHERE idOldNew = 1 AND PriProduct = 1 AND Active = 1 AND idPr = 2");
					$que5 = que("SELECT * FROM products WHERE idOldNew = 1 AND PriProduct = 1 AND Active = 1 AND idPr = 2 ORDER BY ".$gsort." ".$tsort." LIMIT ".$start.",".$perPage);
				}
				elseif ($row4->idTypeProduct2 == '')
				{
					$que_5 = que("SELECT * FROM products WHERE idTypeProduct ='".$row2->idTypeProduct."' AND Active = 1 AND idPr = 2 AND idOldNew = 1 AND PriProduct = 1");
					$que5 = que("SELECT * FROM products WHERE idTypeProduct ='".$row2->idTypeProduct."' AND Active = 1 AND idPr = 2 AND idOldNew = 1 AND PriProduct = 1 ORDER BY ".$gsort." ".$tsort." LIMIT ".$start.",".$perPage);
				}
				else
				{
					$que_5 = que("SELECT * FROM products WHERE idTypeProduct ='".$row2->idTypeProduct."' AND Active = 1 AND idPr = 2 AND idTypeProduct2 = '".$row4->idTypeProduct2."' AND idOldNew = 1 AND PriProduct = 1");
					$que5 = que("SELECT * FROM products WHERE idTypeProduct ='".$row2->idTypeProduct."' AND Active = 1 AND idPr = 2 AND idTypeProduct2 = '".$row4->idTypeProduct2."' AND idOldNew = 1 AND PriProduct = 1 ORDER BY ".$gsort." ".$tsort." LIMIT ".$start.",".$perPage);
				}
				...
				if ($num == 0) echo 'Chưa có sản phẩm KHUYẾN MÃI tại phân mục này !';
