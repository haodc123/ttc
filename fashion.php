    	...
    	<div id="primary">
            ...
            <div class="div_frame_ctrl">
                <ul class="ul_frame_ctrl">
					...
					if ($_REQUEST['flo_menu'] <> '')
					{
						$_SESSION['lo'] = $_REQUEST['flo_menu'];
					}
					...
					?>
                    >
                    	<a href="fashion.php?type=<?php echo $row->NameNone; ?>"><?php echo $row->Name; ?></a>
                    </li>
					<?php
					}
                    ?>
                ...
                <div class="div_frame_ctrl_content">
					...
					while ($row3 = mysql_fetch_object($que3))
					{
						...
						if ($gtype2 == $NameNone)
							echo '<a id="current" href="fashion.php?type='.$gtype.'&type2='.$NameNone.'">'.$Name2.'</a>';
						else
							echo '<a href="fashion.php?type='.$gtype.'&type2='.$NameNone.'">'.$Name2.'</a>';
					}
					?>
                    <div class="clear"></div>
                </div>
            ...
            <div class="div_direction">
                ...
                <select class="flomenu" name="flo_menu" onchange="MM_jumpMenu('parent',this,0)">
                	<option value="<?php echo $_SESSION['url']; ?>">Tất cả</option>
               	<?php
					$que_lo = que("SELECT * FROM location");
					while ($row_lo = mysql_fetch_object($que_lo))
					{
					if ($_SESSION['lo'] == $row_lo->Name)
						echo '<option value="'.$_SESSION['url'].'" selected>'.$row_lo->Name.'</option>';
					else
						echo '<option value="'.$_SESSION['url'].'">'.$row_lo->Name.'</option>';
					}
					?>
                </select>
			...
			<div class="div_products">
				...
				if ($_SESSION['lo'] == 'Tất cả' || $_SESSION['lo'] == '')
				{
					...
					{
						$que_5 = que("SELECT * FROM products WHERE idOldNew = 1 AND PriProduct = 1 AND Active = 1");
						$que5 = que("SELECT * FROM products WHERE idOldNew = 1 AND PriProduct = 1 AND Active = 1 ORDER BY ".$gsort." ".$tsort." LIMIT ".$start.",".$perPage);
					}
					elseif ($row4->idTypeProduct2 == '')
					{
						$que_5 = que("SELECT * FROM products WHERE idTypeProduct ='".$row2->idTypeProduct."' AND Active = 1 AND idOldNew = 1 AND PriProduct = 1");
						$que5 = que("SELECT * FROM products WHERE idTypeProduct ='".$row2->idTypeProduct."' AND Active = 1 AND idOldNew = 1 AND PriProduct = 1 ORDER BY ".$gsort." ".$tsort." LIMIT ".$start.",".$perPage);
					}
					else
					{
						$que_5 = que("SELECT * FROM products WHERE idTypeProduct ='".$row2->idTypeProduct."' AND Active = 1 AND idTypeProduct2 = '".$row4->idTypeProduct2."' AND idOldNew = 1 AND PriProduct = 1");
						$que5 = que("SELECT * FROM products WHERE idTypeProduct ='".$row2->idTypeProduct."' AND Active = 1 AND idTypeProduct2 = '".$row4->idTypeProduct2."' AND idOldNew = 1 AND PriProduct = 1 ORDER BY ".$gsort." ".$tsort." LIMIT ".$start.",".$perPage);
					}
				}
				else
				{
					if ($row2->idTypeProduct == '')
					{
						$que_5 = que("SELECT * FROM products WHERE idOldNew = 1 AND PriProduct = 1 AND Active = 1 AND idLocation = '".$idLocation."'");
						$que5 = que("SELECT * FROM products WHERE idOldNew = 1 AND PriProduct = 1 AND Active = 1 AND idLocation = '".$idLocation."' ORDER BY ".$gsort." ".$tsort." LIMIT ".$start.",".$perPage);
					}
					elseif ($row4->idTypeProduct2 == '')
					{
						$que_5 = que("SELECT * FROM products WHERE idTypeProduct ='".$row2->idTypeProduct."' AND Active = 1 AND idOldNew = 1 AND PriProduct = 1 AND idLocation = '".$idLocation."'");
						$que5 = que("SELECT * FROM products WHERE idTypeProduct ='".$row2->idTypeProduct."' AND Active = 1 AND idLocation = '".$idLocation."' AND idOldNew = 1 AND PriProduct = 1 ORDER BY ".$gsort." ".$tsort." LIMIT ".$start.",".$perPage);
					}
					else
					{
						$que_5 = que("SELECT * FROM products WHERE idTypeProduct = '".$row2->idTypeProduct."' AND Active = 1 AND idLocation = '".$idLocation."' AND idTypeProduct2 = '".$row4->idTypeProduct2."' AND idOldNew = 1 AND PriProduct = 1");
						$que5 = que("SELECT * FROM products WHERE idTypeProduct ='".$row2->idTypeProduct."' AND Active = 1 AND idLocation = '".$idLocation."' AND idTypeProduct2 = '".$row4->idTypeProduct2."' AND idOldNew = 1 AND PriProduct = 1 ORDER BY ".$gsort." ".$tsort." LIMIT ".$start.",".$perPage);
					}
				}
