...
?>
			...
			<div class="div_products">
				...
				// End Paging 1
					...
					$que_5 = que("SELECT * FROM posts WHERE Name LIKE '%".$pquery ."%' OR Content LIKE '%".$pquery ."%'");
					$que5 = que("SELECT * FROM posts WHERE Name LIKE '%".$pquery ."%' OR Content LIKE '%".$pquery ."%' ORDER BY Date DESC LIMIT ".$start.",".$perPage);

					//$que_5 = que("SELECT * FROM products WHERE PriProduct = 1 AND idPost = '".$row_1->idPost."'");
					//$que5 = que("SELECT * FROM products WHERE PriProduct = 1 AND idPost = '".$row_1->idPost."' ORDER BY Date DESC LIMIT ".$start.",".$perPage);
				...
				if ($num == 0) echo 'Không tìm thấy sản phẩm nào!';
				while ($row5 = mysql_fetch_object($que5))
				{
				$que__5 = que("SELECT * FROM products WHERE PriProduct = 1 AND idPost = '".$row5->idPost."'");
				$row__5 = mysql_fetch_object($que__5);
				?>
                <div class="div_product">
                	<a target="_blank" class="a_img" href="../detail_product.php?name=<?php echo $row5->NameNone; ?>"><img src="../images/<?php echo $row__5->urlImg; ?>" alt="<?php echo $row5->Name; ?>" /></a>
                    <div class="info">
                        ...
                        <h1>
                            ...
                            $que8 = que("SELECT * FROM location WHERE idLocation = '".$row__5->idLocation."'");
                            $row8 = mysql_fetch_object($que8);
                            echo $row8->Name;
                            ?>
                        </h1>
                        <h2>
                        	Giá : <span><?php cleanPrice($row__5->Price); ?> đ</span>
                        </h2>
                        <div class="clear"></div>
                        <h3>
